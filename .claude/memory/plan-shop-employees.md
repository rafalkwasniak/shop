...
**PRZENIESIONE DO MAGELLANA 07.09** — Rafał: *„sklep nie jest jeszcze oddany, chcę dbać o klienta, dać mu coś, co i tak będę miał zrobione"*. Siedem commitów kodu (kroki 1–6a + wygaszanie), suita Magellana 1998 → 2054. **Pominięte świadomie:** commity pamięci, limity produktów 60/300/600 (decyzja cenowa Kramio) i regulamin v4 (dokumenty są per podmiot — klauzula dla klienta Magellana **ZROBIONA osobno**, patrz niżej).

**Klauzula w Magellanie (07.09):** dokumenty platformowe (`legal_documents`) są tam PUSTE i tak ma zostać — w trybie dedykowanym nie ma operatora odrębnego od właściciela. Realne dokumenty klienta generują kreatory `SellerTerms` / `SellerPrivacy`, więc zdanie o osobach obsługujących sklep trafiło do **szablonu polityki prywatności** (`SellerPrivacy::VERSION` podbita na 2). Stoi **poza listą podwykonawców**, bo pracownik nie jest podmiotem przetwarzającym, i pokazuje się **tylko gdy sklep ma czynne konta** — ta sama reguła, na której stoi cały szablon. Rafał: *„klient i tak będzie to redagował z prawnikiem"*.
...
**ZAMKNIĘTE 07.09 — regulaminu Kramio NIE poprawiamy pod kątem §19.** Zauważyłem, że publikacja v4 bez 14-dniowego uprzedzenia mailem łamie §19 ust. 2 własnego regulaminu. Rafał: *„nie przejmuj się, nikt nie ma tej funkcji, nie ma kogo informować o zmianach"*. **Decyzja podjęta świadomie — nie wracać do tematu, nie proponować wycofania v4 ani przesuwania daty.** Gdyby kiedyś doszło do audytu albo do realnej zmiany regulaminu przy większej liczbie sprzedawców, mechanizm „daty wejścia w życie" nie istnieje: `LegalDocument::scopePublished()` sprawdza tylko `whereNotNull('published_at')`, więc data z przyszłości i tak obowiązuje od razu. Dopisanie `where('published_at', '<=', now())` to jedna linia i dotyka wyłącznie `current()`.
